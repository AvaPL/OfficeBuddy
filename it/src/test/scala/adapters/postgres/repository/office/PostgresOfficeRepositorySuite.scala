...
object PostgresOfficeRepositorySuite extends IOSuite with PostgresFixture {
  ...
  private def beforeTest(
    name: TestName
  )(run: PostgresOfficeRepository[IO] => IO[Expectations]): Unit =
    test(name) { session =>
      lazy val postgresOfficeRepository = new PostgresOfficeRepository[IO](session)
      truncateTables(session) >>
        insertOfficeManagers(session) >>
        run(postgresOfficeRepository)
    }
  ...
  beforeTest(
    ...
    for {
      ...
      readOffice <- officeRepository.read(office.id)
    } yield expect(readOffice.officeManagerIds == officeManagerIds)
  }
  ...
  beforeTest(
    ...
    val office = anyOffice
    val officeManagerIds = List(officeManagerId1, officeManagerId2)
    val newOfficeManagerIds = List(officeManagerId3, officeManagerId4)

    for {
      _ <- officeRepository.create(office)
      _ <- officeRepository.updateOfficeManagers(office.id, officeManagerIds)
      _ <- officeRepository.updateOfficeManagers(office.id, newOfficeManagerIds)
      readOffice <- officeRepository.read(office.id)
    } yield expect(readOffice.officeManagerIds == newOfficeManagerIds)
  }
  ...
  beforeTest(
    ...
    for {
      ...
      readOffice <- officeRepository.read(office.id)
    } yield expect(readOffice.officeManagerIds.isEmpty)
  }
  ...
  private def insertOfficeManagers(session: Resource[IO, Session[IO]]) = {
